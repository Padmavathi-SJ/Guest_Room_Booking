...
const validateForm = () => {
  ...
  if (!formData.email || !formData.password || !formData.mobile_num) {
  ...
  if (!/^[0-9]{10,15}$/.test(formData.mobile_num)) {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (isLogin) {
      // Store all necessary data including name
      // In the login success handler:
const customerDataToStore = {
  user_id: response.data.customer.user_id,  // Match your database field name
  name: response.data.customer.name,
  email: response.data.customer.email,
  mobile_num: response.data.customer.mobile_num,
  location: response.data.customer.location || '', // Add if available
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="mobile_num" className="block text-sm font-medium text-gray-700">
                Mobile Number
              </label>
              <div className="mt-1 relative rounded-md shadow-sm">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
